...
export const error4xx: RequestHandler = async (
  ...
  try {
    ...
    if (
      ...
      return res
        .status(404)
        .json({ message: "No 4xx data available from Prometheus" });
    }
...
export const error5xx: RequestHandler = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
) => {
  try {
    console.log("Error 5xx method in error4xx controller trigger");
    ...
    if (
      ...
      console.warn("No valid 5xx data from Prometheus.");
      return res
        .status(404)
        .json({ message: "No 5xx data available from Prometheus" });
    }

    const firstVal = data.data.result[0]?.value;
    const error5xx = firstVal ? parseFloat(firstVal[1]) : 0;
    ...
    const point = new Point("5xx")
      .tag("endpoint", "error5xx")
      .floatField("5xx", error5xx);

    writeApi.writePoint(point);
    await writeApi.flush();

    console.log("5xx data written to InfluxDB for user:", username);
    ...
    console.error("Error in error5xx controller", err);
    return next(err);
  }
};
